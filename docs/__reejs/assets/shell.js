...
if (!window.Import) window.Import = async function (url) {
    ...
    if(__USE_STATIC && url.startsWith("/src") && (url.endsWith(".tsx") || url.endsWith(".jsx") || url.endsWith(".ts"))) {
        //replace the last .tsx with .js
        url = url.replace(/\.tsx$/, ".js").replace(/\.jsx$/, ".js").replace(/\.ts$/, ".js");
    }
...
ree.init = async function (options) {
    ...
    let loader = async () => {
        ...
        if (ree.needsHydrate || ree.opts.mode == "csr" || ree.opts.mode == "static" || __USE_STATIC) {
            $("#app").innerHTML = "";
    ...
    if (ree.opts.twindCSR || __USE_STATIC) {
